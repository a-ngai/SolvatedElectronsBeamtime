...
def test_multithread_filtering_template():
    with (open("examples/py_scripts/basic_filtering.py", 'r') as f, 
        open("examples/py_scripts/_temp_multithread_filtering.py", 'w') as g):
        new_lines = []
    ...
    try:
        from examples.py_scripts import _temp_multithread_filtering
    except Exception as e:
        # if os.path.exists("examples/py_scripts/_temp_multithread_filtering.py"): os.remove("examples/py_scripts/_temp_multithread_filtering.py")
        raise e
    if os.path.exists("examples/py_scripts/_temp_multithread_filtering.py"): os.remove("examples/py_scripts/_temp_multithread_filtering.py")